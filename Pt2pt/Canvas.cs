    ...
    public class Canvas
    {
        ...
        public void Render(Scene scene)
        {
            ...
            for (int f = 0; f < scene.Figures.Count; f++)
            {
                ...
                {
                    g.FillPolygon(Brushes.DarkCyan, scene.Figures[f].Pts.ToArray());
                    g.DrawPolygon(Pens.Goldenrod, scene.Figures[f].Pts.ToArray());
                    g.FillEllipse(Brushes.Purple, scene.Figures[f].Pts[scene.Figures[f].Pts.Count - 1].X - 3, scene.Figures[f].Pts[scene.Figures[f].Pts.Count - 1].Y - 3, 6, 6);
                    g.FillEllipse(Brushes.Black, scene.Figures[f].Centroid.X-3, scene.Figures[f].Centroid.Y-3, 6, 6);//*/
                }
            }
            pct.Invalidate();
        }
